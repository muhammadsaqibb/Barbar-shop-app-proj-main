...
const MobileExpenseCard = ({ expense, onEdit, onDelete, formatPrice }: { expense: Expense, onEdit: (expense: Expense) => void, onDelete: (expenseId: string) => void, formatPrice: (amount: number) => string }) => (
    <Card>
        <CardHeader>
            <CardTitle className="text-lg">{expense.name}</CardTitle>
            <CardDescription>{formatPrice(expense.amount || 0)}</CardDescription>
        </CardHeader>
